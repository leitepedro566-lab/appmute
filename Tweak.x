...
- (void)performVolumeChangeToMute {
    ...
    if (avCtrl) {
        // 仅覆盖 Audio/Video（映射控制中心媒体音量），绝不影响 Ringtone（铃声）等通道
        NSArray *categories = @[@"Audio/Video"];
...
// 仅 iOS 14-15 存在此方法，在 16-17 中 Logos 注入时会自动忽略，不会造成任何崩溃
